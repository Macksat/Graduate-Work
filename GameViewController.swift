...
class GameViewController: UIViewController, AVAudioPlayerDelegate {
    ...
    func showTitlePage() {
        ...
        if let scene = self.scene {
            ...
            let i = SKSpriteNode(imageNamed: "卒研展示物タイトル画面") 
        ...
        if let scene = self.scene {
            ...
            if self.textIndex < self.texts[sceneIndex].count {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now()) {
                    ...
                    bubble.size = CGSize(width: scene.size.width - 100, height: scene.size.height / 5)
...
extension GameViewController: MCSessionDelegate, MCNearbyServiceAdvertiserDelegate, MCNearbyServiceBrowserDelegate {
